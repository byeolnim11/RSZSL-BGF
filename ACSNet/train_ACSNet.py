...
def add_similarity_train(parser):
    ...
    parser.add_argument('--dim_att', type = int, default = 85)
    parser.add_argument('--path_att_txt', type = str, default = '85')
    parser.add_argument('--cos_sim', action = 'store_true', default = False)
    parser.add_argument('--att_head', action = 'store_true', default = False)
    return parser
...
def main():
    ...
    args.exp_name += f'GANSimilarity_{args.exp_name}_t{args.target_domain}_beta{args.beta}_{args.source_set}2{args.target_set}_{os.path.basename(args.data_path)}'\
                     ...
                     f'_{"cosine_" if args.cos_sim else ""}{datetime.datetime.now().strftime("%m%d%H%M")}'
    ...
    att = torch.Tensor(preprocessing.normalize(np.loadtxt(args.path_att_txt))).cuda()
    # category2att = {i: att[i] for i in range(len(att))}

    simnet = GANSimilarityNet(args).cuda()
    simnet.backbone.module.resnet.avgpool = nn.AvgPool2d(kernel_size = 7, stride = 1, padding = 0)
    att_classifier = nn.DataParallel(AttributeHead(args, simnet.diff_dim)).cuda()
    ...
    for epoch in range(args.num_epoch):
        # a: clean base; b: noisy novel
        train_meter, domain_meter, cls_loss_avg = train(args, epoch, simnet, att_classifier, domain_classifier,
                                                        A_loader, B_loader, att)

        val_meter = evaluation(args, epoch, simnet, base_similarity_test_loader, att)
        test_meter = evaluation(args, epoch, simnet, novel_similarity_test_loader, att)
        ...
        saver.save_model_if_best(test_acc, att_classifier,
...
def train(args, epoch, simnet, att_classifier, domain_classifier, A_loader, B_loader, att):
    ...
    att_optimizer = optim.SGD(att_classifier.parameters(), lr = domain_lr, momentum = 0.9, weight_decay = args.wd)
    domain_optimizer = optim.SGD(domain_classifier.parameters(), lr = domain_lr, momentum = 0.9, weight_decay = args.wd)

    att_criterion = nn.CrossEntropyLoss().cuda()
    ...
    for batch_i, (A_data, B_data) in tqdm(enumerate(zip(A_pairs_data_loader, B_pairs_data_loader))):
        ...
        att_loss = att_criterion(att_pred, att_targets)
        att_loss.backward() if args.att_head else None
        ...
        domain_meter.update(domain_pred.cpu(), domain_targets.cpu())
...
def evaluation(args, epoch, model, data_loader, att):
    ...
    with torch.no_grad():
        for batch_i, (images, categories, im_names) in tqdm(enumerate(data_loader)):
            ...
            targets = make_cosine_similarities(categories, att) if args.cos_sim else make_binary_similarities(
                categories)
            meter.update(predictions, targets.argmax(1).cpu() if args.cos_sim else targets)
    log(meter.report())
    return meter

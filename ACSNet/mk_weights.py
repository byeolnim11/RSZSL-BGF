...
def main():
    ...
    model.backbone.module.resnet.avgpool = nn.AvgPool2d(kernel_size = 7, stride = 1, padding = 0)
    model.reset_gpu()

    batch_size = 100
    model.eval()

    save_path = f'{args.save_dir}{"_inter_cla" if args.inter_cla and args.save_dir.find("inter_cla") == -1 else ""}'
    os.makedirs(save_path, exist_ok = True)

    # feats sampled from base cla for inter-class similarity calculation
    base_train_set = data_helper._get_clean_base_set()
    base_image_paths = []
    for i, _ in base_train_set.image_list:
        base_image_paths.append(i)
    base_image_paths = random.choices(base_image_paths, k = 500)
    base_feats = []
    with torch.no_grad():
        for i in range(0, 500, batch_size):
            paths = base_image_paths[i:i + batch_size]
            images = [test_transforms(Image.open(path).convert('RGB')) for path in paths]
            feat = model.backbone(torch.stack(images).cuda())
            base_feats.append(feat.data.cpu())
        base_feats = torch.cat(base_feats)
        ...
        for c, image_list in image_dict.items():
            ...
            if not os.path.exists(f'{save_path}/{noisy_set.int2category[c]}_matrix.pth'):
                ...
                for i in tqdm(range(L)):
                    paired = pair_feature(p = feats[i], Q = base_feats) if args.inter_cla else pair_feature(
                        p = feats[i], Q = feats)

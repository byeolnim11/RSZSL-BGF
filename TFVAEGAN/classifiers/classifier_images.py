...
class CLASSIFIER:
    # train_Y is interger 
    def __init__(self, _train_X, _train_Y, _weight, data_loader, _nclass, _cuda, _lr = 0.001, _beta1 = 0.5,
                 _nepoch = 20, _batch_size = 512, generalized = True, netDec = None, dec_size = 4096,
                 dec_hidden_size = 4096):
        _batch_size = max(min(_train_X.shape[0] // 10, _batch_size), 1)
        self.train_X = _train_X.clone()
        self.train_Y = _train_Y.clone()
        self.weight = _weight.clone()
        ...
        self.weight_batch = torch.FloatTensor(_batch_size)
        self.lr = _lr
        self.beta1 = _beta1
        ...
        if self.cuda:
            ...
            self.weight_batch = self.weight_batch.cuda()
    ...
    def fit_zsl(self):
        ...
        for epoch in range(self.nepoch):
            for i in range(0, self.ntrain, self.batch_size):
                ...
                batch_input, batch_label, batch_weight = self.next_batch(self.batch_size)
                self.input.copy_(batch_input)
                self.label.copy_(batch_label)
                self.weight_batch.copy_(batch_weight)

                inputv = Variable(self.input)
                labelv = Variable(self.label)
                output = self.model(inputv)
                loss = self.criterion(output, labelv)
                loss = loss * self.weight_batch
                loss = loss.mean()
                mean_loss += loss.item()
                loss.backward()
                self.optimizer.step()
    ...
    def fit(self):
        ...
        for epoch in range(self.nepoch):
            ...
            # print(self.test_seen_label, self.seenclasses)
            # print(acc_seen, acc_unseen, H)
            if H > best_H:
                best_seen = acc_seen
    ...
    def next_batch(self, batch_size):
        ...
        if start + batch_size > self.ntrain:
            ...
            if rest_num_examples > 0:
                ...
                w_rest_part = self.weight[start:self.ntrain]
            # shuffle the data
            perm = torch.randperm(self.ntrain)
            self.train_X = self.train_X[perm]
            self.train_Y = self.train_Y[perm]
            ...
            w_new_part = self.weight[start:end]
            # print(start, end)
            if rest_num_examples > 0:
                return torch.cat((X_rest_part, X_new_part), 0), torch.cat((Y_rest_part, Y_new_part), 0), torch.cat(
                    (w_rest_part, w_new_part))
            else:
                return X_new_part, Y_new_part, w_new_part
        else:
            self.index_in_epoch += batch_size
            end = self.index_in_epoch
            # print(start, end)
            # from index start to index end-1
            return self.train_X[start:end], self.train_Y[start:end], self.weight[start:end]
    ...
    def val_gzsl(self, test_X, test_label, target_classes):
        ...
        for i in range(0, ntest, self.batch_size):
            ...
            if self.cuda:
                inputX = Variable(test_X[start:end].cuda(), requires_grad = False)
            else:
                inputX = Variable(test_X[start:end], requires_grad = False)

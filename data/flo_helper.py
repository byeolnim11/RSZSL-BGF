...
class FLOHelper(DataHelper):
    ...
    def __init__(self, args):
        super(FLOHelper, self).__init__(args)
        self.name = 'Animal'
        # todo
        self.base_categories = read_split_from_file(f'{self.root_path}/meta/trainclasses.txt')
        self.novel_categories = read_split_from_file(f'{self.root_path}/meta/testclasses.txt')
        att_splits = sio.loadmat(f'{self.root_path}/origin/res101.mat')
        img_names = [i[0].split('Flowers/')[-1] for i in att_splits['image_files'].squeeze()]
        labels = [i - 1 for i in att_splits['labels'].squeeze()]
        classes = np.loadtxt(f'{self.root_path}/origin/classes.txt', dtype = str)[:, 1].tolist()
        assert len(labels) == len(img_names)
        self.img_name2category = {img_names[i]: classes[labels[i]] for i in range(len(labels))}
        return

    def _get_novel_test_set(self):
        return SourceTestSet(self.root_path, self.novel_categories, self.img_name2category, self.test_transforms)

    def _get_base_test_set(self):
        return SourceTestSet(self.root_path, self.base_categories, self.img_name2category, self.test_transforms)

    def _get_clean_novel_set(self):
        return SourceTrainSet(self.root_path, self.novel_categories, self.img_name2category, self.train_transforms)

    def _get_clean_base_set(self):
        return SourceTrainSet(self.root_path, self.base_categories, self.img_name2category, self.train_transforms)
...
class SourceTrainSet(DataSet):
    def __init__(self, root_path, categories, img_name2category, transform = None, max_num = 1000):
        super(SourceTrainSet, self).__init__(root_path, categories, transform)

        train_ids = []
        # todo
        for line in open(os.path.join(root_path, 'origin', 'train_test_split.txt')).readlines():
        ...
        self.image_list = []
        # todo
        for line in open(os.path.join(root_path, 'origin', 'images.txt')).readlines():
            img_id, img_path = line.strip().split(' ')
            img_name = img_path.split('Flowers/')[-1]
            category = img_name2category[img_name]
            if img_id in train_ids:
                if category in categories:
                    if category_count[category] >= max_num:
                        continue
                    # todo
                    self.image_list.append((os.path.join(root_path, 'origin', img_path),
...
class SourceTestSet(DataSet):
    def __init__(self, root_path, categories, img_name2category, transform = None):
...
class WebSet(DataSet):
    ...
    def __init__(self, root_path, categories, transform = None, max_noisy_images_per = None):
        ...
        for category_name in categories:
            ...
            # todo
            dir_path = os.path.join(self.root_path, 'FLO_web', web_name)
            if not os.path.exists(dir_path):
                print(category_name)
                print(dir_path)
                raise FileNotFoundError

...
class AWA2Helper(DataHelper):
    ...
    def __init__(self, args):
        super(AWA2Helper, self).__init__(args)
        self.name = 'Animal'
        # todo
        # self.base_categories = read_split_from_file('data/meta/AWA2/trainvalclasses.txt')
        self.base_categories = read_split_from_file('data/meta/AWA2/trainclasses.txt')
        self.novel_categories = read_split_from_file('data/meta/AWA2/testclasses.txt')
        return
...
class SourceTrainSet(DataSet):
    def __init__(self, root_path, categories, transform = None, max_num = 1000):
        ...
        train_ids = []
        # todo
        for line in open(os.path.join(root_path, 'Animals_with_Attributes2', 'train_test_split.txt')).readlines():
        ...
        self.image_list = []
        # todo
        for line in open(os.path.join(root_path, 'Animals_with_Attributes2', 'images.txt')).readlines():
            ...
            if img_id in train_ids:
                if category in categories:
                    if category_count[category] >= max_num:
                        continue
                    # todo
                    self.image_list.append((os.path.join(root_path, 'Animals_with_Attributes2', 'JPEGImages', img_path),
...
class WebSet(DataSet):
    ...
    def __init__(self, root_path, categories, transform = None, max_noisy_images_per = None):
        ...
        for category_name in categories:
            ...
            # todo
            dir_path = os.path.join(self.root_path, 'AWA2_web', web_name)
            if not os.path.exists(dir_path):
                print(category_name)
                print(dir_path)
                raise FileNotFoundError
            ...
            self.image_list += category_list[:max_noisy_images_per]
            # print(category_name, category_list, sep = '\n')
